...
2. 원리 뽀개기: 현상이 왜/어떻게 일어나는지의 메커니즘 자체(절차, 인과관계) — 그 현상을 둘러싼
   ...
   **이 축은 인과관계 주장이 핵심이라 4단계의 근거 인용 규칙이 다른 어떤 축보다 강하게 적용된다.**
   "실종 사건은 초기 대응이 중요하다", "이런 조치는 신뢰를 떨어뜨릴 수 있다"처럼 근거 없이도
   그럴듯하게 들리는 문장으로 시작하거나 채우면 안 된다 — 이런 문장이 하나라도 들어갈 것
   같으면, 쓰기 전에 반드시 법령·매뉴얼·전문가·통계·판례 중 하나를 web_search로 찾는다.
   못 찾았다면 이 축을 아예 만들지 않는다 — 못 찾았는데도 "~할 수 있다"류 일반론으로 채워서
   억지로 만드는 것보다, 그 자리에 다른 근거 있는 축을 넣거나 축 개수를 줄이는 쪽이 낫다.
...
4. 과거 썰: 조직의 노선·역사, 정책 어젠다의 역사적 기원, 사건 자체의 타임라인·경위. (신호: 과거로 거슬러 올라가야 이해되는 상황, 장기 진행형 이슈)
   ...
   검색 결과(특히 참고할 기사 제목)에 금액·날짜·인원수 같은 구체적 수치가 나왔다면, references
   목록에만 남겨두고 넘어가지 않는다 — 그 수치를 explanation 본문 문장 안으로 직접 가져와서
   쓴다. "OO 혐의를 받았다"보다 "OO원(개월치 등 구체적 단위)을 받았다는 혐의를 받았다"처럼
   쓰는 쪽이 항상 낫다.
...
- 축 하나당 4~7문장. 짧게 요약하고 끝내지 말고, 구체적인 수치·날짜·경위·인용을 풀어서
  충분히 설명한다 — 다만 늘어난 분량은 새로운 근거·세부사실로 채워야지, 이미 한 말을
  다른 표현으로 반복하거나 당연한 부연설명으로 채우지 않는다.
