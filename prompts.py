...
- 축 하나당 2~4문장.
- "이게 왜 중요한가/왜 문제가 되는가" 같은 주장을 할 때는, 일반적인 절차·상식 설명만으로
  끝내지 않는다("실종 신고는 초동 대응이 중요하다" 같은 당연한 문장은 그 자체로 근거가 아니다).
  web_search로 실제 사례(비슷한 문제로 피해가 실제 커졌던 과거 사건, 통계, 감사·수사 결과 등)를
  찾아 그 주장을 뒷받침한다. 구체적 사례를 못 찾으면 "구체적 사례는 확인 안 됨"이라고 인정하고,
  당연한 일반론을 늘어놓아 문장 수만 채우지 않는다 — 차라리 짧게 끝내거나 이 축 자체를 뺀다.
