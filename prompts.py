...
4. 과거 썰: 조직의 노선·역사, 정책 어젠다의 역사적 기원, 사건 자체의 타임라인·경위. (신호: 과거로 거슬러 올라가야 이해되는 상황, 장기 진행형 이슈)
   ...
   실종자·피해자처럼 사건의 중심 인물이 있다면, 그 사건이 파생시킨 후속 이슈(예: 관련자 문책,
   비슷한 다른 사건 전수조사, 제도 개선 논의)보다 그 인물 본인의 사건 경위(실종·피해 일시·장소·
   정황, 현재 발견/해결 여부)를 먼저 담는다. 이건 선택이 아니라 필수 절차다 —
   이름·지역·날짜처럼 원문에 나온 단서를 조합해 최소 한 번은 반드시 web_search로 찾아본 뒤에만
   "공개된 정보가 없다"고 판단할 수 있다. 검색을 안 해보고 "정보가 부족하다"고 넘어가지 않는다.
   검색 결과에 구체적 경위(며칠 몇 시에 어디서 마지막으로 목격/연락됐는지 등)가 나오면 그걸
   반드시 이 축에 담고, 정말 검색해봐도 안 나올 때만 "실종 경위는 아직 자세히 공개되지 않음"
   이라고 쓴다. 파생 이슈로 이 축을 대체하지 않는다 — 필요하면 파생 이슈는 별도 축으로 다룬다.
